...
const toNavaCode = inp => inp.split``.map(char => pickRandomFromArray(englishIndex[char.toLowerCase()])).join` `;
const fromNavaCode = inp => inp.split` `.map(word => navajoIndex[word]).join``;
// .join` `.replace(/ ([.!?])/g, '$1')
