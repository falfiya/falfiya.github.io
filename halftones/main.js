...
const settings = {},
  ...
  meanOfChunk = pipes([getImageData, clearRect], 0, collectColors, meanColor, [computeBrightness, 0], [[0, computeDarknessFromBrightness], 1], [[0, Math.round], 1], (...a) => a);
...
function init() {
  ...
  Matrix.slowmap(Matrix.fillNeew(timesX, timesY), (v, x, y) => {
    const M = meanOfChunk(x * settings.size, y * settings.size),
      brightness = M[0],
      color = M[1],
      percentOfSize = brightness / 255,
      adjustedColor = color.slice(0, 3).map(v => Math.ceil(v - (circleSquareDifference / 3)));
    svg.append("circle")
      .attr("cx", x * settings.size + settings.size / 2) // YOu may n0w fixx
      .attr("cy", y * settings.size + settings.size / 2)
      .attr("r", (Number(settings.size)) / 2)
