...
A._arity = (n, fn) => {
  if (!Number.isInteger(n) || !(Math.sign(n) + 1)) throw new Error(`The first argument to _arity must be a non-negative integer instead of ${n}!`);
...
A._annotateFn = (fn, options) => {
  if (typeof fn !== "function") throw new Error("To annotate a function, you need a function");
  ...
  if (o.showArgnames) o.argnames.forEach((v, i) => displayArgs[i] = `${displayArgs[i] || ""}:${v}`);

  let counter = "";
  if (o.showCounter) counter = `, @ ${o.values.filter(v => v !== A.__).length} / ${o.arity}`;
  ...
  if (o.showDescriptors) {
    const descriptorsArray = [],
      pastDescriptors = fn[Symbol.for("descriptors")] || {},
      currentDescriptors = { ...pastDescriptors, ...o.descriptors};
  ...
  if (displayArgs.length && o.showCounter) newFn.displayName = `${descriptorsString}${o.name}(${displayArgs.join(", ")}${counter})`;
  else newFn.displayName = o.name;

  newFn[Symbol.for("values")] = o.values;
  if (o.showArgnames) newFn[Symbol.for("argnames")] = o.argnames;
...
A._curryN = (n, fn, name, argnames = [], o = {}) => {
  ...
  if (n > 30) throw new Error(`${n} is a pretty big number. Are you sure you wanted to do that?`);

  if (A.type(fn) !== "function") throw new Error("The second argument to _curryN must be a function!");
  ...
  get: (f, key) => {
    const argsGetter = (...kargs) => {
      const objReceiver = (obj) => {
        let nobj;
        if (f.flag) nobj = f(f.flag, obj);
        else nobj = f(obj);

        const value = nobj[key];
        if (typeof value === "function") return nobj[key](...kargs);
...
A.uncurryN = A._curryN(2, (n, fn) => {
  const mari = (...a) => {
    ...
    newFn = A._annotateFn(mari, {
      arity: n,
      name: fn.name,
      showCounter: false,
      showArgnames: false,
    }),
    d = newFn[Symbol.for("descriptors")];
  d.curried = false;
  d.uncurried = true;
...
A.flip = (fn) => {
  ...
  if (fnargs) oargnames = fnargs;
  else oargnames = A.argnames(fn);

  if (oargnames.length < 2) o.showArgnames = false;
  // Something didn't work. Don't show the argnames
  else nargnames = [oargnames[1], oargnames[0], ...oargnames.slice(2)];

  o.descriptors = {flipped: true};
...
A._Number = {
  times(fn) {
    return Array(Number(this)).fill(0).map((v, i) => fn(i));
  },
};
A._ArrayLike = {
  last() {
