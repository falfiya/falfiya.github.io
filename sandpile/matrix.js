class Matrix {
  ...
  set(x, y, v) {
    if (this.existsy(y)) this.values[y][x] = v;
    else (this.values[y] = [])[x] = v;
  ...
  remove(x, y) {
    if (this.exists(x, y)) {
      if (this.values[y].length - 1) delete this.values[y][x];
      else delete this.values[y];
  ...
  log() {
    ...
    for (var i = 0; i < copy.length; i++) {
      copy[i] = copy[i] || [];
      for (var j = 0; j < copy[i].length; j++) copy[i][j] = copy[i][j] || this.default;

    }
    const str = copy.map(a => a.map(v => "0".repeat(3 - ((String(v)).length)) + v).join(" ")).join("\n");
    console.log(str);
    return this;
  }
